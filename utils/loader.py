...
class Loader:
    ...
    def load_pickle(self, pickle_id):
        ...
        # given in parameter
        file_path = self.__out_path + "/data_" + str(pickle_id) + ".pkl"

        if os.path.isfile(file_path):
            df = pd.read_pickle(file_path)
        else:
            raise Exception(
                "The pickle file data_{}.pkl doesn't exist".format(pickle_id)
            )
        return df
    ...
    def random_pickles(self, n_pickles=3, init=42, verbose=True):
        ...
        pickle_files = [name for name in glob.glob(self.__out_path + "/data_*.pkl")]
        # draw p_files
        seed(init)
        ...
        if n_pickles <= 6:
            ...
            raise Exception(
                "The parameter n_pickles ("
                + "{}) exceed the numbers of pickle files ({})".format(
                    n_pickles, len(pickle_files)
                )
            )
